...
func GetData(databaseServerAddress string, databaseServerPort string) http.HandlerFunc {

	return func(w http.ResponseWriter, r *http.Request) {
		body, res, err := getContainerEvents(databaseServerAddress, databaseServerPort)
		if err != nil {
		...
		jsonData, err := transform(res, databaseServerAddress, databaseServerPort)
		if err != nil {
...
func getContainerEvents(databaseServerAddress string, databaseServerPort string) ([]byte, *InfluxData, error) {

	body, res, err := getByURI("http://" + databaseServerAddress + databaseServerPort + "/query?db=flowDB&&q=SELECT%20*%20FROM%20ContainerEvents")
	if err != nil {
...
func getFlowEvents(databaseServerAddress string, databaseServerPort string) ([]byte, *InfluxData, error) {

	body, res, err := getByURI("http://" + databaseServerAddress + databaseServerPort + "/query?db=flowDB&&q=SELECT%20*%20FROM%20FlowEvents")
	if err != nil {
		return nil, nil, fmt.Errorf("Error: Resource Unavailabe %s", err)
	}
...
func deleteContainerEvents(id []string, databaseServerAddress string, databaseServerPort string) ([]Nodes, error) {
	var node Nodes
	var nodes []Nodes

	for i := 0; i < len(id); i++ {
		_, err := http.Get("http://" + databaseServerAddress + databaseServerPort + "/query?db=flowDB&q=DELETE%20FROM%20%22ContainerEvents%22%20WHERE%20(%22EventID%22%20=%20%27" + id[i] + "%27)")
		if err != nil {
			return nil, fmt.Errorf("Error: Reading from Resoponse %s", err)
		}
	}

	body, res, _ := getContainerEvents(databaseServerAddress, databaseServerPort)
...
func transform(res *InfluxData, databaseServerAddress string, databaseServerPort string) (*GraphData, error) {
	var nodes []Nodes
	var links []Links
	var node Nodes
	var err error
		...
		if res.Results[0].Series[0].Name == "ContainerEvents" {
			for j := 0; j < len(res.Results[0].Series[0].Values); j++ {
				if res.Results[0].Series[0].Values[j][2].(string) != "delete" {
					...
					nodes, err = deleteContainerEvents(id, databaseServerAddress, databaseServerPort)
					if err != nil {
						return nil, fmt.Errorf("Error: Reading from Resoponse %s", err)
					}
				}
	...
	links, err = generateLinks(nodes, databaseServerAddress, databaseServerPort)
	if err != nil {
...
func generateLinks(nodea []Nodes, databaseServerAddress string, databaseServerPort string) ([]Links, error) {

	_, res, err := getFlowEvents(databaseServerAddress, databaseServerPort)
	if err != nil {
